...
export function ShareTargetClient() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // PDFs: send as-is for now (OCR route expects image; fallback message)
      const isPdf =
      ...
      if (isPdf) {
        setStatus("error");
        setError(
          "Por ahora solo procesamos imágenes (capturas o fotos de comprobantes). Los PDF de resumen de tarjeta usalos desde Importar.",
        );
        return;
      }
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50/80 py-12 dark:border-zinc-800 dark:bg-zinc-900/40">
          ...
          <p className="text-sm text-zinc-500">
            {status === "loading"
              ...
              : "Extrayendo monto, fecha y comercio…"}
          </p>
        </div>
      )}
