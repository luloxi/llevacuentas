...
export function CompartidoView() {
  ...
  if (loading && months.length === 0) {
    return (
      <p className="text-sm text-zinc-500">Cargando gastos compartidos…</p>
    );
  }
  if (error) {
    return (
    ...
    <div className="space-y-4">
      ...
      {summary && (
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-violet-200/80 bg-violet-50/50 p-4 dark:border-violet-900/40 dark:bg-violet-950/20">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-violet-800/80 dark:text-violet-300/80">
              Total compartido
            </p>
            <p className="mt-1 text-xl font-bold tabular-nums text-violet-950 dark:text-violet-100">
              {formatArs(summary.totalArsCombined)}
            </p>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-4 dark:border-zinc-800 dark:bg-zinc-900/50">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-zinc-500">
              Por persona (partes iguales)
            </p>
            <p className="mt-1 text-xl font-bold tabular-nums">
              {formatArs(summary.fairSharePerMember)}
            </p>
            <p className="text-xs text-zinc-500">
              {summary.memberCount}{" "}
              {summary.memberCount === 1 ? "miembro" : "miembros"}
            </p>
          </div>
          <div className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-4 dark:border-zinc-800 dark:bg-zinc-900/50">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-zinc-500">
      ...
      {members.length > 0 && (
        <section className="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-800">
          <h2 className="text-sm font-semibold">Quién registró / pagó</h2>
      ...
      {period === "all" && months.length > 0 && (
        <div className="space-y-3">
          ...
          {months.map((m) => (
            <details
              key={m.period}
              className="group rounded-2xl border border-zinc-200 open:shadow-sm dark:border-zinc-800"
            >
