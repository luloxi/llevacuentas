      ...
      <div className="w-full max-w-sm space-y-8 text-center">
        ...
        {googleEnabled && (
          ...
          >
            <button
              type="submit"
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm font-medium shadow-sm hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:hover:bg-zinc-800"
            >
              <GoogleIcon />
        ...
        <form
          action={async (fd) => {
            ...
            await signIn("dev-login", {
              email: String(fd.get("email") || "demo@llevacuentas.local"),
              name: String(fd.get("name") || "Demo"),
              redirectTo: "/onboarding",
            });
          }}
          className="space-y-3 rounded-2xl border border-zinc-200 p-4 text-left dark:border-zinc-800"
        >
          <p className="text-xs font-medium uppercase tracking-wide text-zinc-500">
            {googleEnabled ? "O acceso demo" : "Acceso demo (configurá Google OAuth en .env)"}
          </p>
          <input
            name="name"
            defaultValue="Demo"
          ...
          <input
            name="email"
            type="email"
            defaultValue="demo@llevacuentas.local"
            placeholder="Email"
            className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900"
          />
          <button
